...
int avl[7];
map <string, int> dcd;
string enc[] = {"S", "M", "L", "XL", "XXL", "XXXL"};
vector <int> parse(string &s) {
    string cur;
    vector <int> ans;
    s += ',';
    for (char e : s) {
        if (e == ',') ans.push_back(dcd[cur]), cur.clear();
        else cur += e;
    }
    return ans;
}
int main() {
    ...
    string s;
    cin >> n;
    int nodes = n + 8, src = n + 6, snk = n + 7;
    Dinic :: init(nodes);
    for (int i = 0; i < 6; i++) Dinic :: addEdge(n + i, snk, avl[i]);
    for (int i = 0; i < n; i++) {
        cin >> s;
        for (int e : parse(s)) Dinic :: addEdge(i, n + e, 1);
        Dinic :: addEdge(src, i, 1);
    }
    auto flow = Dinic :: maxFlow(src, snk);
    if (flow != n) {
        cout << "NO\n";
    } else {
        cout << "YES\n";
        for (int i = 0; i < n; i++) {
            for (int idx : Dinic :: adj[i]) if (Dinic :: edges[idx].flow > 0) {
                    cout << enc[Dinic :: edges[idx].u - n] << '\n';
                }
        }
    }
}
